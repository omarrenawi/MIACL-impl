...
def sign_eval(trials, num_of_signers):
    # token generation
    trial_res=list()
...
def vrfy_eval(trials, num_of_signers):
    # token verification 
    trial_res=list()
    for _ in range(trials):
        ec_point, fq, max_int_size = (BN128Point, BN128FQ, 32)
...
if __name__=="__main__":
        ...
        hash = lambda x: sha256(x).digest()
        trials=10
        eval_file= open("snowblind-eval.csv", "w")
        eval_file.write("#Signers,Issuance,Token Verification\n")
        for i in num_of_signers:
            sign= sign_eval(trials, i)
            vrfy= vrfy_eval(trials, i)
            out = f"{i},{sign},{vrfy}\n"
            eval_file.write(out)
        eval_file.close()